...
You can start using UIC with the theme we provide, but at some point, you might want to apply custom values to the design tokens and align them with your brand.
To solve that, UIC has a command-line interface for creating new themes.
...
All other values are inherited from the default UIC theme.
...
Now that we have a config file with theme definitions added, we can use UIC CLI to generate these themes.
...
We can start by picking the `productTheme` theme we've just built and pass it to the `UIC` provider:
...
It still uses other tokens from the default UIC theme, which means Button component uses a violet color for its background.
...
- `@type` comment defines the type for the variable used next. This means that the config format will be type checked according to the type definition coming from the UIC package.

...
In addition to themes, `reshaped.config.js` allows you to create theme fragments.
...
Running `yarn build:themes` or `npm run build:themes` now will take your added theme definitions from `reshaped.config.js` file and compile them into the `src/themes` folder.

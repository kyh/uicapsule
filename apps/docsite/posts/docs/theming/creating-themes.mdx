...
You can start using UICapsule with the theme we provide, but at some point, you might want to apply custom values to the design tokens and align them with your brand.
To solve that, UICapsule has a command-line interface for creating new themes.
...
All other values are inherited from the default UICapsule theme.
...
Now that we have a config file with theme definitions added, we can use UICapsule CLI to generate these themes.
...
We can start by picking the `productTheme` theme we've just built and pass it to the `UICapsule` provider:

```code
import { UICapsule } from "@uicapsule/components";
import "themes/productTheme/theme.css";

const Application = ({ children }) => <UICapsule theme="productTheme">{children}</UICapsule>;
```

Our product now uses a custom theme and has a new `foregroundNeutral` token value available.
It still uses other tokens from the default UICapsule theme, which means Button component uses a violet color for its background.
...
- `@type` comment defines the type for the variable used next. This means that the config format will be type checked according to the type definition coming from the UICapsule package.

...
const meta: DocsMeta = {
	title: "Badge",
	description: "Compact element that represents the status of an object or user input.",
	componentImport: 'import { Badge } from "reshaped";',
	typeImport: 'import type { BadgeProps } from "reshaped";',
	storybookUrl: "https://reshaped.so/storybook/?path=/story/components-badge",
	properties: {
		base: {
			children: { type: ControlType.slot, control: { mode: "text", defaultValue: "23" } },
			color: {
		...
		container: {
			children: { type: ControlType.slot, control: { defaultValue: true, ignore: true } },
			position: {
				type: ControlType.enum,

...
const SectionFigmaPlugin = (props: T.Props) => {
	...
	const [intendedMode, setIntendedMode] = React.useState<typeof colorMode>("light");
	const [cursorStatus, setCursorStatus] = React.useState<"idle" | "hidden" | "visible">("idle");
		...
		<ThemeProvider theme={selectedTheme}>
			...
			>
				...
				<Container width={embed ? "820px" : "1000px"}>
					<View direction={{ s: "column", l: "row" }} gap={{ s: 18, l: 14 }} align="center">
								...
								>
									<View padding={3} gap={2} direction="row" align="center">
										...
										<View.Item>
											<Button.Aligner className={s.close}>
												<Button startIcon={IconClose} variant="ghost" size="small" />
											</Button.Aligner>
										</View.Item>
									</View>
									<View padding={3} direction="row" gap={3}>
										<Text variant="body-strong-2">Themes</Text>
										<Actionable href={pluginLink} attributes={{ target: "_blank" }}>
									...
									<View.Item grow className={s.dots}>
										<View padding={3} gap={2}>
											...
											<Card padding={3} href={pluginLink} attributes={{ target: "_blank" }}>
												<View direction="row" align="center" gap={3}>
													...
													<DropdownMenu.Trigger>
														{(attributes) => (
															<Button
																...
																startIcon={intendedMode === "dark" ? IconMoon : IconSun}
																endIcon={IconChevronDown}
															>
																{intendedMode === "dark" ? "Dark mode" : "Light mode"}
															</Button>
														)}
													</DropdownMenu.Trigger>
													<DropdownMenu.Content>
														<DropdownMenu.Item onClick={() => setIntendedMode("light")}>
															Light mode
														</DropdownMenu.Item>
														<DropdownMenu.Item onClick={() => setIntendedMode("dark")}>
															Dark mode
														</DropdownMenu.Item>
													</DropdownMenu.Content>
												</DropdownMenu>

...
import { View, Button, useToggle } from "reshaped";
...
const getPropertyString = (args: {
	...
	const isStringValue = type === ControlType.string || type === ControlType.enum;
	...
	if (isSlotBoolean || isSlotString) {
		const resolvedValue = resolveSlotValue({ value, control: property.control });
	...
	if (type === ControlType.array) {
		const resolvedValue = (value as unknown[])
			?.map((item) =>
				getPropertyString({ value: item, property: property.item, objectNotation: true })
			)
			.filter(Boolean)
			.join(", ");
	...
	return [
		key,
		key && (objectNotation ? ": " : "="),
		objectNotation || isStringValue || isSlotString ? formattedValue : `{${formattedValue}}`,
	]
		.filter(Boolean)
		.join("");
};
...
const ArticleDemo = (props: {
	...
	children: (props: Record<string, { props: string; children: React.ReactNode }>) => string;
}) => {
			...
			[sectionName]: {
				...
				children:
					resolveSlotValue({
						...
						control: child?.type === ControlType.slot && child.control ? child.control : undefined,
					}) || "",
			},
		};
	}, {});

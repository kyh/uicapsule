	...
	React.useEffect(() => {
		...
		const handleScroll = () => {
			...
			els.forEach((el, index) => {
				...
				if (elRect.top >= 0 && elRect.bottom <= document.documentElement.clientHeight) {
			...
			if (lastScrolledByIndex !== null && !activeIndices.includes(lastScrolledByIndex)) {
				activeIndices.push(lastScrolledByIndex);
			}

			const sorted = activeIndices.sort((a, b) => a - b);
			const selection: [number, number] = [sorted[0], sorted[sorted.length - 1]];
	...
	return (
		<View gap={2} as="ul" align="start" className={s.root}>
			...
			{items.map((item, index) => {
				const isActive = from !== undefined && to !== undefined && index >= from && index <= to;
				...
				return (
					...
					>
						<Link href={item.url} color={isActive ? "primary" : "inherit"} variant="plain">
							{item.text}
						</Link>
					</Text>
				);

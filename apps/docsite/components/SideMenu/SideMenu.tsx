...
import { MenuItem, Text, TextField, View, Icon, Badge } from "reshaped";
...
const debouncedTrackEvent = debounce(() => {
	ga.trackEvent({ category: ga.EventCategory.Documentation, action: "documentation_search" });
}, 300);
...
const SideMenuItem = (props: T.MenuItemProps) => {
	...
	const linkElement = (
		<MenuItem
			...
			endSlot={
				data.soon && (
					<Badge size="small" variant="outline" color={selected ? "primary" : undefined}>
						Soon
					</Badge>
				)
			}
...
const SideMenuGroup = (props: T.MenuGroupProps) => {
	...
	return (
		<>
			...
			<motion.div
				initial={!opened ? { opacity: 0, height: 0 } : { opacity: 0, height: "auto" }}
				animate={
					opened
						? {
								opacity: 1,
						...
						: { opacity: 0, height: 0, overflow: "hidden", margin: "0 -16px", padding: "0 16px" }
				}
				transition={{ duration: 0.5, ease: [0.04, 0.62, 0.23, 0.98] }}
			>
				<View gap={1}>
			...
			<MenuItem.Aligner>
				<View gap={1}>
					{props.normalizedItems.map((child, index) => {
						...
						return <SideMenuItem {...child} key={child.title || index} fullWidth={fullWidth} />;
					})}
				</View>
			</MenuItem.Aligner>
		</View>
		...
		<View gap={6} className={s.root}>
			...
			{sections.map((item, index) => {
				if (item.url || item.onClick) {
					return (
						<View.Item gapBefore={index === 0 ? undefined : 0} key={item.title || index}>
							<MenuItem.Aligner>

...
import { Switch, TextField, View } from "reshaped";
import PropertyBaseControl from "../PropertyBase/PropertyBaseControl";
import * as T from "../Properties.types";

const isControlModeText = (props: T.SlotControlProps): props is T.SlotTextControlProps => {
		...
		<PropertyBaseControl name={name} hideName={hideName}>
			{isControlModeText(props) ? (
				<TextField
					name={name}
					value={props.value || ""}
					onChange={({ name, value }) => props.onChange({ name, value: value || "" })}
				/>
			) : (
				<View padding={[2, 0]}>
					<Switch

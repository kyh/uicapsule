...
import { Button, Dismissible, Modal, Hidden, View, useToggle } from "reshaped";
...
const useMenu = (): MenuItem[] => {
	return [
		{ title: "Documentation", url: "/content/docs/getting-started/overview", desktop: true },
...
const HeaderMenuMobile = () => {
	...
	const menu = useMenu();
	const { deactivate: closeMenu, toggle: toggleMenu, active: menuActive } = useToggle();
		...
		extraItems.push(
			{
				...
				url: "https://github.com/reshaped/community/issues",
				onClick: () => {
					ga.trackEvent({ category: ga.EventCategory.External, action: "external_submit_request" });
				},
			},
			{
				title: "Sign out",
	...
	return (
		<Hidden hide={{ s: false, l: true }}>
			...
			<Modal active={menuActive} onClose={closeMenu} position="end" size="280px" padding={3}>
				<View gap={3}>
						...
						menu.map((item) => {
							...
							return (
								<Link href={item.url} passHref key={item.title}>
									<Button variant="ghost" className={className} onClick={item.onClick}>
										{item.title}
									</Button>
								</Link>
							);

...
import { Actionable, Badge, Text, View, Link, Button, Tabs, TabsProps } from "reshaped";
...
const PostMeta = (props: any) => {
	const { data } = props;
	const { componentImport, typeImport, storybookUrl, relatedComponents, title } = data;

	if (!componentImport && !typeImport && !storybookUrl && !relatedComponents) return null;
...
export default function PostPage(props: any) {
	...
	const isComponentRoute =
		currentRouteChunks.includes("components") || currentRouteChunks.includes("utilities");
	const subRouteName = currentRouteChunks[currentRouteChunks.length - 1];
	const isComponentSubRoute =
		isComponentRoute && (subRouteName === "props" || subRouteName === "examples");
	...
	const handleTabChange: TabsProps["onChange"] = ({ value }) => {
		...
		const targetRoute = value === "documentation" ? parentRoute : `${parentRoute}/${value}`;
										...
										<Button
											...
											onClick={() => {
												ga.trackEvent({
													...
													action: "external_notify_component",
												});
											}}
										>
											Get notified
						...
						{tabs?.length && (
							<View.Item gapBefore={6}>
								<Tabs
									...
									value={isComponentRoute && !isComponentSubRoute ? tabs[0].value : subRouteName}
								>
									<Tabs.List>
										{tabs.map((tab: any) => (
											<Tabs.Item
...
export const getStaticProps = async ({ params }: any) => {
	...
	const contentFileParentPath = contentFilePath.split("/").slice(0, -1).join("/");
	...
	const hasExamples = fs.existsSync(path.join(contentFileParentPath, "examples.mdx"));
	...
	return {
		props: {
			...
			source: mdxSource,
			meta: { ...frontMatter, ...meta?.default, tabs: frontMatter.tabs || tabs },
			shareImage: fs.existsSync(shareImagePath) ? `/img/og/${slugPath}.png` : null,
		},
	};
};

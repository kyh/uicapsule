...
const HOME = { x: STAGE_WIDTH - HEAD - EDGE_PAD, y: 120 };
...
export const ChatHeads = () => {
  ...
  const settle = (offsetX: number, offsetY: number, velocityX: number, velocityY: number) => {
    ...
    void controls.start({
      transition: { bounce: 0.35, duration: 0.55, type: "spring" },
      x: snapX,
      y: snapY,
    });
  };
  ...
  const restore = () => {
    ...
    void controls.start({
      opacity: 1,
      scale: [0.3, 1],
      transition: { bounce: 0.45, duration: 0.5, type: "spring" },
      x: positionRef.current.x,
      y: positionRef.current.y,
    });
  };
      ...
      <AnimatePresence>
        {dragging && (
          <motion.div
            ...
            transition={{ bounce: 0.3, duration: 0.35, type: "spring" }}
            ...
            <AnimatePresence>
              {unread > 0 && (
                <motion.span
                  ...
                  animate={{ scale: 1 }}
                  exit={{ scale: 0 }}
                  transition={{ bounce: 0.5, duration: 0.35, type: "spring" }}

...
const nearestStage = (value: number) => {
  if (value < 0.5) {
    return 0;
  }
  if (value < 1.5) {
...
const TILES: Tile[] = [
  { icon: Wifi, id: "wifi", label: "Internet", on: true },
  { icon: Bluetooth, id: "bt", label: "Bluetooth", on: true },
  { icon: Flashlight, id: "torch", label: "Torch", on: false },
  { icon: Moon, id: "dnd", label: "Do Not Disturb", on: false },
  { icon: RotateCcw, id: "rotate", label: "Auto-rotate", on: true },
  { icon: Moon, id: "dark", label: "Dark theme", on: true },
];
...
export const NotificationShade = () => {
  ...
  const release = () => {
    ...
    void animate(progress, nearestStage(progress.get()), {
      bounce: 0.24,
      duration: 0.5,
      type: "spring",
    });
    ...
    >
      ...
      >
        ...
        <div
          ...
          onPointerDown={(event) => {
            draggingRef.current = { startProgress: progress.get(), startY: event.clientY };

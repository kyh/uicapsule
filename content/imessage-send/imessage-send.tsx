...
const INITIAL_MESSAGES: Message[] = [
  { id: 1, sent: false, text: "Ready to see the send animation?" },
  { id: 2, sent: true, text: "Born ready" },
  { id: 3, sent: false, text: "Type something — or long-press a bubble for tapbacks" },
];
...
export const ImessageSend = () => {
  ...
  // The draft's layoutId must match the bubble it becomes, so the next id is render state.
  const [draftId, setDraftId] = useState(100);
  ...
  const send = (text: string) => {
    ...
    setMessages((previous) => [...previous.slice(-7), { id: draftId, sent: true, text: trimmed }]);
  ...
  return (
    <MotionConfig reducedMotion="user">
      <div className="relative flex h-[620px] w-[340px] flex-col overflow-hidden rounded-[44px] bg-black shadow-2xl shadow-black/60 ring-8 ring-black select-none">
                ...
                <AnimatePresence>
                  {message.reaction && (
                    <motion.span
                      ...
                      animate={{ scale: 1, y: 0 }}
                      exit={{ scale: 0 }}
                      transition={{ bounce: 0.55, duration: 0.4, type: "spring" }}
                ...
                <AnimatePresence>
                  {reactingTo === message.id && (
                    <motion.div
                      ...
                      transition={{ bounce: 0.4, duration: 0.38, type: "spring" }}

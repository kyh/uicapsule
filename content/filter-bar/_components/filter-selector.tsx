...
import type { ElementType as ReactElementType, ReactElement } from "react";
import {
  ArrowLeftIcon,
  ArrowRightIcon,
  ChevronRightIcon,
...
export const FilterableColumn = <TType extends ColumnDataType>({
  column,
  setProperty,
  actions,
  filters,
  ...
  const handleSelect = () => {
    if (column.type === "boolean") {
      actions.setFilterValue({ columnId: column.id, type: "boolean", values: [true] });
      return;
    }
      ...
      property && column && column.type !== "boolean" ? (
        ...
        <Command
          ...
          filter={(itemValue, search, keywords) => {
            const extendValue = `${itemValue} ${keywords?.join(" ")}`;

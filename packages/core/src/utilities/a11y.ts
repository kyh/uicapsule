...
export const isKeyboardMode = () => document.body.hasAttribute(keyboardModeAttribute);
	...
	return (root: HTMLElement, options: TrapOptions = {}): Trap => {
		...
		const release: Release = (releaseOptions = {}) => {
			...
			if (releaseTargetTrigger) {
				releaseTargetTrigger.focus({ preventScroll: withoutFocusReturn || !isKeyboardMode() });
			}

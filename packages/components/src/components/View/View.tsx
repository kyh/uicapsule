...
const View = <As extends keyof JSX.IntrinsicElements = "div">(
  ...
  const renderDivider: T.RenderDivider = ({ className, key }) => {
    ...
    if (typeof direction === "string" && direction.startsWith("row")) {
      ...
      const viewports = Object.keys(direction) as Array<
        keyof G.ResponsiveOnly<string>
      >;
      isDividerVertical = viewports.reduce((acc, viewport) => {
        const viewportDirection = (direction as G.ResponsiveOnly<T.Direction>)[
          viewport
        ];
  ...
  const renderItem: T.RenderItem = ({ className, child, index }) => {
    ...
    const dividerElement =
      !!index && divided && renderDivider({ className, key });
    let itemElement;

...
let counter = 0;
const generateId = () => `__uic-toast-${counter++}`;
...
const toastReducer: T.Reducer = (state, action) => {
  ...
  switch (action.type) {
    case "add":
      const { position = "bottom-end", ...toastProps } =
      ...
      return {
        ...
        [position]: [
          ...state[position],
          { id: action.payload.id, toastProps, status: "entering" },
        ],
      };
    ...
    case "remove":
      ...
      positions.forEach((position) => {
        nextState[position] = nextState[position].filter(

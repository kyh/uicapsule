...
const settle = async (onClick: (() => void | Promise<void>) | undefined, onSettled: () => void) => {
  try {
    await onClick?.();
  } finally {
    onSettled();
...
export const GlobalAlertDialog = () => {
  ...
  const runAndClose = (
    ...
    setPending(action);
    void settle(onClick, () => setPending(null));
  };
  ...
  const onOpenChange = (open: boolean) => {
    ...
    if (!open) {
      runAndClose(alertState.cancel?.onClick, "cancel");
    }
  };

  const onConfirm = () => {
    runAndClose(alertState.action?.onClick, "action");
  };
